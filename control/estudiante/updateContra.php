...
if (isset($_POST['idHdn']) && !empty($_POST['idHdn'])) {
    ...
    if ($num > 0 ){
        if ($nuevaPass == $confPass){
            ...
            if ($ejecutar){
                ...
                header("Location: ../../vistas/estudiante/cambiarContra.php?bien=$bien");
            }else{
                $err = "No se pudo realizar la operación. Error:" .mysqli_error($conexion);
                header("Location: ../../vistas/estudiante/cambiarContra.php?err=$err");
            }
        }else{
    ...
    header("Location: ../../vistas/estudiante/cambiarContra.php");
}
